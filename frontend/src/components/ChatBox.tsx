  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: 'system',
      content:
        'Welcome to AI Travel Planner! Fill out the form on the left to generate a travel plan or ask questions here.',
      timestamp: new Date(),
    },
  ]);
          ...
          content: (
            <div className="space-y-2">
              ...
              <p>
                Your{' '}
                {itinerary.dates.end
                  ? `${getDaysBetween(itinerary.dates.start, itinerary.dates.end)}-day`
                  : ''}{' '}
                trip includes:
              </p>
              ...
              <p>
                Total budget: ${formatNumber(itinerary.budget.total)} • Estimated cost: $
                {formatNumber(itinerary.budget.spent || 0)}
              </p>
              <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                Click the button below to view the detailed itinerary
              </p>
            </div>
          ),
          timestamp: new Date(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (framework === 'mastra-ai') {
        ...
        await sendChatMessageWithMastra(userInput, itinerary, streamedText => {
  ...
  return (
    <div className="sleek-card chat-container" style={{ height: '100%' }}>
      <div className="sleek-card-header">
        ...
        <div
          className={`framework-badge ${getBadgeClass(framework)} selected`}
      ...
      <div
        ...
        style={{ overflowY: 'auto', height: 'calc(100% - 114px)', padding: '1rem' }}
      >
